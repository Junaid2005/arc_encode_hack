                ...
                if liquidation_count > 0:
                    ...
                    if days_since_last is not None:
                        if days_since_last < 30:
                            recency_penalty_pct = (
                                0.20  # -20% if liquidated in last 30 days
                            )
                        elif days_since_last < 60:
                            recency_penalty_pct = (
                                0.15  # -15% if liquidated in last 60 days
                            )
                        elif days_since_last < 90:
                            recency_penalty_pct = (
                                0.10  # -10% if liquidated in last 90 days
                            )
                    ...
                    liquidation_penalty = (
                        count_penalty_pct + recency_penalty_pct + severity_penalty_pct
                    )
                    # Cap total liquidation penalty at -50%
                    liquidation_penalty = min(0.50, liquidation_penalty)

                    recency_str = (
                        f"{days_since_last:.0f} days since last"
                        if days_since_last is not None
